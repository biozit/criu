...
static int prepare_pstree_ids(pid_t pid)
{
	...
	list_for_each_entry(item, &root_item->children, sibling) {
		...
		if (leader->pid->state != TASK_UNDEF) {
			...
			if (__pid < 0)
				break;
			helper = lookup_create_item(__pid);
			if (helper == NULL)
				return -1;
	...
	for_each_pstree_item(item) {
		...
		__pid = pstree_pid_by_virt(item->pgid);
		if (__pid->state != TASK_UNDEF) {
			BUG_ON(__pid->state == TASK_THREAD);
			rsti(item)->pgrp_leader = __pid->item;
			continue;
		}
